...
def create_game():
    ...
    try:
        ...
        game_id = game_repo.get_gameID(game_key)  # Retrieve the game ID based on the game key
...
def get_questions(game_id):
    try:
        questions = question_repo.get_random_questions(5)
        if not questions:
            return jsonify({'error': 'Not enough questions available'}), 400

        question_repo.assign_questions_to_game(game_id, questions)
...
def submit_answer():
    ...
    try:
        is_correct = question_repo.check_answer(question_id, answer)
        if is_correct:
            player_repo.increment_score(player_id)
            question_repo.mark_question_as_played(game_id, question_id)
